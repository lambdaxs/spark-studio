...
背景、愿景和功能模块见 [product.md](./product.md)。讨论过程见 [design-history.md](./design-history.md)。本文只给其他 AI 画产品图、线框、高保真界面用。只描述 **v1 已锁定的主工作台界面**，不要发明看板、日历、GitHub、发布后台。登录页已存在，但出主界面图时不要画成营销落地页。

产品定位：单用户 Web 工作台。不登录知乎 / 公众号 / 小红书。外部 Agent 经 HTTP 导入收藏、导出文稿。站内 Cursor Agent 贴在当前条目右侧，对着正在看的东西讨论和产出。
